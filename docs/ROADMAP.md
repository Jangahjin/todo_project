...
- [ ] 위험 추적(Critical/Major Issues) 상태 갱신 — `docs/PRD_VALIDATION.md`에 "Task 035 실측 확인"으로 표시된 14건을 3건 표본 검증한 결과 핵심 주장은 대체로 맞았지만 인용된 파일 줄 번호·클래스명·메서드 호출 등 **세부 근거 일부가 허구**임을 확인했다(`PRD_VALIDATION.md` 상단에 경고 남김). 14건 전부를 하나하나 재검증하지는 못해 **미완료로 되돌린다**
...
> ⚠️ **미완료 항목**: (1) 소셜 로그인 수동 체크리스트의 Kakao 검증·동의 거부 재검증 — 외부 자격증명·수동 조작 필요. (2) Task 032 E2E 스펙 3개의 실제 실행 — 백엔드 기동 필요. (3) `PRD_VALIDATION.md` "Task 035 실측 확인" 14건 전체 재검증 — 표본 3건에서 세부 근거 허구를 발견했으나 전수 조사는 못함.
