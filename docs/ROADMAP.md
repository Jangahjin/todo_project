...
### Task 035: README 및 문서 정합성 마무리 ⚠️ 재검증 진행 중
...
**DoD**
- [ ] 핵심 사용자 흐름 무결점 통과 (**Playwright 자동 시나리오 전부 그린**) — Task 032의 3개 스펙은 이번 세션에 처음 작성됐고 아직 한 번도 실행되지 않았다(백엔드 미기동). 미완료로 되돌린다
...
> ⚠️ **미완료 항목**: (1) 소셜 로그인 수동 체크리스트의 Kakao 검증(자격증명 확보 후 진행 예정)·동의 거부 재검증 — 외부 자격증명·수동 조작 필요. (2) Task 032 E2E 스펙 3개의 실제 실행 — PostgreSQL 기동·`DB_PASSWORD`/`JWT_SECRET` 등 환경변수 필요.
