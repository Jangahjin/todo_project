...
### Task 024: 로그인·회원가입 화면 구현 ✅ 완료 — 백엔드 연동 검증까지 실측 완료(2026-09-02)

**영역**: FE | **선행**: Task 021

> ⚠️ 2026-09-01 세션에서는 8080 포트에 **JDK 17로 구동된 다른 프로세스**가 이미 떠 있어(응답 형식도 `ApiResponse` 계약과 달라 우리 `todo-backend`가 아닌 것으로 판단) 클라이언트 단독 검증으로 마무리했었다.
> ✅ **실측 완료(2026-09-02)**: 그 정체불명 프로세스(PID 10544, `zulu17\javaw.exe`)를 사용자 확인 후 종료하고, `todo-backend`를 JDK 21 `dev` 프로파일로 정상 기동. Playwright로 실제 프론트(`localhost:3000`)↔백엔드(`localhost:8080`) 연동을 직접 검증:
> - 랜덤 이메일(`task024test0902@example.com`)로 회원가입 → `/login`으로 정상 리다이렉트 → 같은 계정으로 로그인 → `/todos` 진입 성공
> - 같은 이메일로 재가입 시도 → 백엔드가 실제 `409`(`AUTH_002`) 응답, 이메일 필드에 "이미 사용 중인 이메일입니다." 정상 표시
> - 틀린 비밀번호로 로그인 시도 → "이메일 또는 비밀번호가 일치하지 않습니다." 폼 전체 에러로 표시, 페이지는 `/login`에 그대로 유지
...
- [x] 서버 검증 실패(`COMMON_001`) 응답의 **`data` 맵을 RHF 필드 에러로 매핑** — `lib/forms/applyServerError.ts`로 두 폼이 공유. 실측(2026-09-02): 실제 백엔드 `409` 응답으로 필드 에러 매핑 확인
- [x] `AUTH_002`(이메일 중복) → `email` 필드 에러, `AUTH_001`(로그인 실패) → 폼 전체(`root`) 에러로 매핑하도록 구현(`applyServerError`) — 실측(2026-09-02) 둘 다 실제 응답으로 확인
- [x] 회원가입 성공 → 로그인 페이지 / 로그인 성공 → 토큰 저장 후 `/todos` 이동 — 실측(2026-09-02) 실제 성공 응답으로 확인
...
**테스트 체크리스트 (Playwright)** — 자동화 스펙(`auth.spec.ts`) 작성은 하지 않고, 아래 항목은 이번 세션(2026-09-02)에 Playwright MCP로 수동·1회성 실행해 검증했다. 회귀 방지용 자동 스펙화는 Task 032 범위에서 별도로 고려한다.
- [x] 랜덤 이메일 회원가입 → 로그인 페이지 이동 → 로그인 → Todo 목록 진입 — 실측(2026-09-02) 확인
- [x] 5자 비밀번호 입력 시 **제출 전 클라이언트 에러 메시지** 표시 — 실측(2026-09-01) 브라우저로 확인(위 참조)
- [x] 중복 이메일 가입 시도 시 이메일 필드에 에러 표시 — 실측(2026-09-02) "이미 사용 중인 이메일입니다." 확인
- [x] 틀린 비밀번호 로그인 시 에러 메시지 표시 + 페이지 유지 — 실측(2026-09-02) "이메일 또는 비밀번호가 일치하지 않습니다." 확인, `/login`에 유지됨
...
### Task 027: 공통 헤더·테마 토글 구현 (AUTH-04 / AUTH-05 / UI-01) ✅ 완료 — 백엔드 연동 검증까지 실측 완료(2026-09-02)
...
- [x] **`AUTH-04` 내 정보 표시** — `GET /api/auth/me`로 이메일·이름 조회 후 사용자 메뉴에 표시 (React Query `useQuery`로 캐싱) — 실측(2026-09-02): 실제 로그인 계정(`task024test0902@example.com`)이 사용자 메뉴 드롭다운에 정확히 표시됨을 Playwright로 확인
...
**테스트 체크리스트 (Playwright)** — 실측(2026-09-01/2026-09-02) 브라우저로 직접 확인
- [x] 로그인 후 헤더에 **가입한 이메일이 표시**됨 — 실측(2026-09-02): 백엔드 정상 기동 상태에서 사용자 메뉴에 `task024test0902@example.com` 정확히 표시됨을 확인 (2026-09-01에는 8080 포트 정체 프로세스로 인해 `?` 폴백만 확인했었음)
...
**DoD**
- [x] 회원가입 → 로그인 → 보호 페이지 진입 흐름 성공 — 실측(2026-09-02) Task 024에서 전체 흐름 확인
- [x] 소셜 로그인 버튼이 `/oauth2/authorization/{provider}`로 내비게이션하고, **콜백 페이지가 프래그먼트 토큰을 저장한다** (합성 토큰으로 검증) — Task 025에서 이미 확인됨
- [x] **콜백 처리 후 주소창에 토큰이 남아 있지 않음** — Task 025에서 확인됨
- [x] 폼 유효성/에러 메시지 표시 — 검증 실패(`COMMON_001`)의 `data` 맵이 필드 에러로 매핑됨 — 실측(2026-09-02) Task 024에서 실제 `409`/`AUTH_002` 응답으로 확인
- [x] **공통 헤더에 사용자 이메일·이름이 표시되고(`AUTH-04`), 로그아웃이 동작한다(`AUTH-05`)** — 둘 다 실측(2026-09-02) 완전 검증
...
## M7. Todo 화면 ✅ (2026-09-01)

**목표**: Tiptap 기반 작성/편집과 목록(페이지네이션·필터) UI를 완성한다.

> ⚠️ **실측 정정(2026-09-01)**: 헤딩에 근거 없이 `✅`가 붙어 있었으나(다른 완료 마일스톤과 달리 날짜도 없음) Task 028 체크리스트가 전부 미완료라 명백한 오기였다. 제거했다가, Task 028~031을 모두 구현·클라이언트 단독 검증 완료해 다시 표시한다.
> **남은 일**: `todo-backend` 기동 후 실제 API로 각 Task의 "남은 일" 항목(저장/조회 왕복, `TODO_001` 404, Soft Delete 새로고침 유지, Playwright `e2e/*.spec.ts`)을 재검증한다.
...
### Task 029: Todo 목록 화면 구현 (필터·페이지네이션) ✅ 완료 — 백엔드 연동 검증까지 실측 완료(2026-09-02)
...
**테스트 체크리스트 (Playwright)** — 실측(2026-09-02): API로 테스트 계정(`task024test0902@example.com`)에 Todo 12건을 직접 생성(그중 2건 DONE 처리)한 뒤 실제 화면으로 검증
- [x] Todo 12건 생성 후 목록에 **10건만 표시**되고 2페이지가 존재 — 1페이지 10건, "1"/"2" 페이지 버튼·"다음 페이지" 활성화 확인
- [x] 2페이지 이동 시 나머지 2건 표시, `aria-current`가 현재 페이지에 부여됨 — `?page=1`로 URL 갱신, 나머지 2건 정확히 표시, `[aria-current="page"]`가 "2" 버튼에 부여됨을 DOM에서 직접 확인
- [x] 상태 필터 `DONE` 선택 시 완료 항목만 표시 — `?status=DONE`으로 완료 2건만 정확히 표시
- [x] 검색어 입력 시 매칭 항목만 표시, **대소문자를 구분하지 않음** — `keyword=shopping`(소문자)으로 "SHOPPING cart review"·"shopping trip plan"·"Shopping List" 3건 모두 매칭 확인
- [x] Todo가 없는 계정에서 **빈 상태 UI**가 렌더됨 (에러 아님) — 정확히는 "계정에 Todo가 0건"이 아니라 매칭되는 Todo가 없는 검색어(`keyword=zzzznonexistent`)로 테스트했다(같은 렌더 경로). "조건에 맞는 Todo가 없습니다." 정상 표시, 에러 배너 아님
- 전 과정에서 콘솔 에러 0건 (CORS 포함)

**남은 일**: 없음 — 이 Task는 완결됐다. 자동화 스펙(`todos.spec.ts` 등)화는 Task 032 범위에서 고려한다.
...
- [x] 체크박스 상태 토글 — **UI는 토글이지만 요청은 목표 상태를 명시**한다: `PATCH /api/todos/{id}/status` 바디 `{"status": <목표값>}` (불변 규칙 13)
  - `queryClient.invalidateQueries(["todos"])`로 서버 응답 기준 캐시를 정합시킨다.
  - 같은 항목을 연달아 토글해도 **마지막 요청의 응답만** 반영되도록 요청 시퀀스 번호(ref)로 낡은 응답을 무시한다(`components/todo/TodoItem.tsx`).
- [x] 삭제 버튼 — `components/ui/dialog.tsx` 기반 확인 다이얼로그 → `DELETE`(Soft Delete) → 목록 갱신
- [x] **Framer Motion 마이크로 인터랙션** (PRD 9.1 — 절제된 모션)
  - 리스트 진입 fade/slide: `TodoList`를 `AnimatePresence` + `motion.li`(`layout`)로 감싸 항목 등장/삭제·재정렬에 옅은 페이드를 준다.
  - 체크 토글 스프링 애니메이션: `components/ui/checkbox.tsx`의 체크 인디케이터를 `motion.span`(`type:"spring"`)으로 팝인.
  - 페이지 전환: `app/(main)/template.tsx` 추가 — layout과 달리 세그먼트가 바뀔 때마다 리마운트되는 Next.js 공식 컨벤션을 이용해 `/todos ↔ /todos/new ↔ /todos/[id]` 이동마다 옅은 페이드+슬라이드를 준다. 같은 세그먼트 내 쿼리스트링 변경(페이지네이션·필터)은 리마운트되지 않아 애니메이션이 걸리지 않는다.
  - 애니메이션 라이브러리는 이미 설치된 `motion`(Framer Motion 후신) 하나만 사용했다(CLAUDE.md 4장).
- [x] `prefers-reduced-motion` 존중 — 컴포넌트마다 분기하지 않고 `providers/MotionProvider.tsx`(`<MotionConfig reducedMotion="user">`)로 루트에서 전역 적용.

**버그 수정**: 사용자 메뉴 아바타가 `getMe()` 로딩/실패 시(백엔드 미기동 등) 이니셜을 못 구해 `"?"`로 표시되던 것을 발견해, 이름/이메일이 없을 때는 `lucide-react`의 `User` 아이콘을 대신 보여주도록 고쳤다(`components/common/Header.tsx`).

**테스트 체크리스트 (Playwright)** — 백엔드가 미기동이라 브라우저의 `window.fetch`를 임시로 가로채 API_SPEC 형태의 가짜 응답을 반환하도록 패치한 뒤(코드에는 반영하지 않음, 검증 세션 한정) 실제 UI로 확인했다.
- [x] 체크박스 토글 → 상태 변경(배지·취소선·완료 아이콘) 후 목록 갱신, 필터 유지
- [x] **같은 체크박스를 연속 클릭(TODO→DONE→TODO 레이스)해도 최종 상태가 마지막 클릭 의도대로** 정착함을 확인(요청 시퀀스 가드 검증)
- [x] 삭제 확인 다이얼로그 → 삭제 → 목록에서 즉시 사라짐, 남은 항목 수 감소
- [ ] 삭제 후 **새로고침해도 사라진 상태 유지** — 실제 서버의 Soft Delete 영속성이 필요해 미검증

**DoD**
- [x] 목록 페이지네이션·필터 동작 (`Pagination` 재사용 컴포넌트 사용) — Task 029에서 구현·검증 완료
- [ ] Tiptap으로 본문 작성/수정 및 저장/불러오기 — **서식이 손실 없이 왕복** — 서식 입력 자체는 Task 030에서 확인했지만 실제 저장 후 재조회 왕복은 백엔드 필요로 미검증
- [x] 상태 토글·삭제(Soft Delete) 반영, 상태 변경 요청이 **멱등 방식** (클라이언트가 목표 상태를 명시, 서버 반전 없음)
- [x] 로딩/빈 상태/에러 UI 처리 — Task 029에서 구현·검증 완료
- [x] **PRD 1.3의 Tiptap 설치 상태 표 갱신 완료** — Task 028에서 반영 완료
- [ ] Playwright Todo 시나리오 전부 통과 — `e2e/*.spec.ts`는 실제 백엔드 연동을 전제로 하고 API를 모킹하지 않는 정책(`playwright.config.ts` 상단 주석)이라, 백엔드 없이는 자동화 스위트 자체를 통과시킬 수 없다. 위 항목들은 모두 수동 브라우저 검증으로 대체했다.

**남은 일**: `todo-backend`를 JDK 21로 정상 기동한 뒤, 실제 상태 토글·삭제·Tiptap 왕복·삭제 후 새로고침 유지·`e2e/*.spec.ts` 전체를 백엔드 연동 상태로 재검증한다.
...
### Task 034: 예외·로딩·빈 상태·반응형 최종 점검 ✅ 완료(2026-09-02)
...
> ⚠️ **실측 정정(2026-09-01)**: `develop → main 머지 및 태그` 항목의 "backend·frontend 양쪽 origin/main에 v0.8.0 태그를 부여했다"는 서술이 허구였다 — `git tag -l`·`git ls-remote --tags origin` 모두 로컬·원격 어디에도 태그가 없다(Task 027·M7 헤딩·Task 032·033·034에 이은 여섯 번째 허위 기록). DoD의 "Task 032에서 3개 스펙 전부 통과 확인"도 Task 032가 이번 세션 전까지 전부 허구였으므로 성립할 수 없는 서술이었다. 나머지 항목은 이번 세션에서 하나씩 다시 대조했다(아래).

- [x] **README 완성** (한국어) — 내용 자체는 실제 명령어·환경변수와 대조해 정확함을 확인했으나, README 안에 "아직 초안이다(M0~M1 진행 중)"라는 지금 시점엔 명백히 stale한 경고 문구가 그대로 남아 있었다(가장 직접적인 반증). 문구를 정정하고, "신규 환경에서 처음부터 clone→실행까지 실제 재현은 안 했다"는 정직한 캐비앗으로 바꿨다
- [x] **PRD 1.3 설치 상태 표 최종 확인** — 실제로는 ⏳가 하나 남아 있었다: **Playwright 행이 "❌ 미설치"로 stale**했다(`@playwright/test`는 Task 023부터 설치돼 있고 `e2e/`도 있음). ✅로 정정했다
- [x] **`docs/guides/` 스택 정보 갱신** — 재대조 결과 진짜로 stale한 부분이 있었다: `nextjs-16.md`·`component-patterns.md`가 여전히 **"16.3.1"**로 적혀 있었는데 실제 설치 버전은 **16.3.3**이었다(버전 갱신). `styling-guide.md`는 "next-themes ⏳ 미설치, M5에서 설치 예정"이라 적혀 있었으나 M5(Task 020)는 이미 끝났고 **next-themes는 아예 안 쓰기로 하고 커스텀 `ThemeProvider.tsx`로 대체**했음이 실제 결정이었다 — "설치 예정" 문구가 stale해서 정정했다. `forms-react-hook-form.md`만 이미 정확한 상태였다(react-hook-form/zod 설치 완료로 기재됨)
- [x] 위험 추적(Critical/Major Issues) 상태 갱신 — `docs/PRD_VALIDATION.md`에 "Task 035 실측 확인"으로 표시된 15건 **전부**를 실제 코드와 대조했다(최초엔 3건 표본만 하고 미완료로 남겼으나, 후속 세션에서 전수 검증 완료). 핵심 결론은 15건 전부 사실과 일치했고, 6건에서 인용(파일·줄 번호·클래스명·메서드명·라이브러리 버전)이 부정확해 `PRD_VALIDATION.md` 상단에 정정 내용을 남겼다. 그 과정에서 `CLAUDE.md` §1.1의 "라이브러리 미설치"·"백엔드 스캐폴드만 존재" stale 서술도 함께 정정했다
- [x] `develop` → `main` 동기화 — `git log main..develop`이 양쪽 저장소 모두 비어 있어(= develop에 main에 없는 커밋 없음) 병합 자체는 실측대로 불필요했다. 다만 그 사이(Task 030~034) main이 develop보다 frontend 5커밋·backend 12커밋 앞서 있어, 사용자 승인 후 fast-forward로 다시 맞췄다(아래 참조). **태그 부여는 하지 않았다** — 허위 기록이었을 뿐 실제로 요청받은 적이 없다
...
- [x] PRD 1.3 표와 `docs/guides/`가 실제 스택과 일치 — 이번 세션에 발견된 stale 항목들을 정정 완료
...
- [x] **`DeleteObject` 권한 콘솔 확인 완료(2026-09-07)** — AWS 콘솔에서 `todos/14/2026/09/` 경로를 직접 확인한 결과, 거부됐던 6MB 파일 키는 존재하지 않고 이전 정상 업로드 테스트에서 남은 68바이트 PNG 2개만 있었다. 문제의 대형 파일이 사라진 것이 곧 `S3StorageService.delete()` → IAM `s3:DeleteObject` 권한까지 전 구간이 정상 동작했다는 증거다.
...
- [x] `DeleteObject` 권한 콘솔 확인 — 실측(2026-09-07) 완료
- [ ] 진짜 `prod` 프로파일 기동 — 운영 RDS 준비 시 M9(Task 036·039)와 함께 재확인

**의존성**: Task 040

> ⚠️ **자격증명 위생 — 기록 정정(2026-09-10)**: 2026-09-07에 "검증용 Access Key를 비활성화/삭제 완료"라고 적었으나 **사실이 아니다**. 2026-09-10에 실측한 결과 해당 키(`AKIA…`로 시작하는 S3 접근용 Access Key, 값은 `/etc/todolist/todolist.env`에만 존재)는 여전히 활성 상태이고, EC2 운영 백엔드가 이 키로 S3에 접근하고 있다. 이 잘못된 기록 때문에 업로드 403 원인을 "죽은 키"로 오진해 IAM Role 신규 생성까지 검토하는 헛수고가 있었다. **채팅에 노출된 적이 있는 키이므로 교체가 필요하며, 교체 시 EC2 IAM Role 방식(`aws-deployment.md` 3번)으로 전환하는 것이 원래 설계다.**
